...
WORDLIST_FILENAME = "C:/Users/Akshay Kher/Desktop/6.00.1x/ps_4/words.txt"
...
def isValidWord(word, hand, wordList):
    ...
    """
    hand_new = hand.copy()
    wordList_new = wordList[:]
    flag = True
    if word in wordList_new:
        for i in word:
            flag = flag and (word.count(i)<=hand_new.get(i,0))
        return flag
    else:
        return False

#isValidWord('bow', {'b': 1, 'i': 2, 'k': 1, 'j': 1, 'o': 1, 'w': 1}, wordList)
#
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    if user_input == ".":
        ...
        print "\"%s\" earned %d points. Total: %d points" % (user_input,getWordScore(user_input, n), Total_Score)
        print
...
def playGame(wordList):
    ...
    if user_input =='n':
        hnd = dealHand(HAND_SIZE)
        playHand(hnd,wordList,HAND_SIZE)
        playGame(wordList)
    elif user_input =='r':
        if hnd=="":
